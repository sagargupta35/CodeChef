...
class CodeChef{
    ...
    public static void main (String[] args) throws java.lang.Exception {
        ...
        while(t-- > 0) {
            ...
            out.println(getMaxWeight(weights, costs, k, 0, dp));
        }
        out.close();
    }
    
     static long getMaxWeight(long [] weight, long [] cost,
                     long k, int st, Map<Long, Long> [] dp){
        if(st == weight.length-1){
            if(k >= cost[st]) return weight[st];
            return 0;
        }
        if(k <= 0) return 0;
        if(dp[st] != null && dp[st].get(k) != null) return dp[st].get(k);
        if(dp[st] == null) dp[st] = new HashMap<>();
        long without = getMaxWeight(weight, cost, k, st+1, dp);
        long with = 0;
        if(k >= cost[st]) with = getMaxWeight(weight, cost, k-cost[st], st+1, dp)+weight[st];
        dp[st].put(k, Math.max(with, without));
