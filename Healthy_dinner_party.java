...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int s = in.nextInt();
            String [][] dp = new String[l.length()][s+1];
            String res = getProteinRequirement(s, l, arr, 0, dp);
            if(res == null) out.println("IMPOSSIBLE");
            else out.println(res);
        }
        out.close();
    }
    
    static String getProteinRequirement(int target, String s,
                                        int [] arr, int st, String [][] dp){
        if(target <= 0) return null;
        int cur = s.charAt(st)-'a';
        if(st == s.length()-1){
            if(arr[cur] == target) return ""+s.charAt(st);
            return null;
        }
        if(dp[st][target] != null){
            if(dp[st][target].isEmpty()) return null;
            return dp[st][target];
        }
        String without = getProteinRequirement(target, s, arr, st+1, dp);
        String with = getProteinRequirement(target-arr[cur], s, arr, st+1, dp);
        if(with != null) with = s.charAt(st) + with;
        else if(arr[cur] == target) with = ""+s.charAt(st);
        if(with == null && without == null) {
            dp[st][target] = "";
            return null;
        }
        if(with == null) return dp[st][target] = without;
        if(without == null) return dp[st][target] = with;
        if(with.compareTo(without) > 0) return dp[st][target] = without;
        return dp[st][target] = with;
    }
