...
class CodeChef3 {
        ...
        while (t-- > 0){
            ...
            if(!flag){
                ...
                if(!flag){
                    ...
                    sum += (long) t1 * (n-1) + (long) t2 *n;
                    for (int i = 0; i < s1.length(); i++) {
                        if (s1.charAt(i) == '1') sum += 1;
                    }
                    System.out.println(sum);
                }
            } else {
                for (int i = 0; i < s2.length(); i++) {
                    if (s2.charAt(i) == '1') sum += n;
                }
                for (int i = 0; i < s1.length(); i++) {
                    if (s1.charAt(i) == '1') sum += 1;
                }
    ...
    static class FenwickTreePURQ {
        ...
        long sum(int r){ // computes sum from 0 to r
            long res = 0;
            while (r >= 0){
                res += bit[r];
                r = (r&(r+1))-1;
