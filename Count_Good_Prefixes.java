...
class CodeChef{
    ...
    public static void main (String[] args) throws java.lang.Exception {
        ...
        while(t-- > 0) {
            ...
            int prev = 0;
            for(int i = 0; i<n; i++){
                pref[i] = prev + 'b' - s.charAt(i);
                prev = pref[i];
            }
            prev = 0;
            for(int i = n-1; i>=0; i--){
                suf[i] = prev + 'b'-s.charAt(i);
                prev = suf[i];
            }
            long res = 0;
            for(int i = 0; i<n; i++){
                long pre = pref[i]* 2L - (i+1);
                long aft = 0;
                if(i < n-1) aft = suf[i+1]* 2L - (n-i-1);
                if(pre == 0){
                    if(aft > 0) res += k-1;
                } else if(aft == 0){
                    if(pre > 0) res += k;
                    ...
                    if(pre+aft > 0) res += k;
                    else if(Math.abs(aft) <= pre) res += k;
                    else{
                        long aft2 = Math.abs(aft);
                        long m = (long) Math.ceil((double) pre / ((double) aft2 - (double) pre));
                        res += Math.min(k, m);
                    }
                } else{
                    if(aft > Math.abs(pre)){
                        long m = (long) Math.floor(((double) Math.abs(pre)) / ((double) aft + (double) pre));
                        if(k > m) res += k-m-1;
                    }
                }
            }
