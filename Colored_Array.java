...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int [] arr = ai(n);
            int [][] b = new int [n][];
            for(int i = 0; i<n; i++) b[i] = ai(m);
            int [][] c = new int [n][];
            for(int i = 0; i<n; i++) c[i] = ai(m);
            Integer [][] dp = new Integer[n][k+1];
            int [] max = new int [n];
            for(int i = 0; i<n; i++){
                max[i] = getMax(b, c, arr[i]-1, i);
            }
            System.out.println(maxPoints(arr, b, c, 0, dp, k, max));
        }
        out.close();
    }
    
    static int getMax(int [][] b, int [][] c, int curColor, int idx){
        int max = b[idx][curColor];
        for(int i = 0; i<b[0].length; i++){
            max = Math.max(max, b[idx][i] - c[idx][i]);
        }
        return max;
    }
    
    static int maxPoints(int [] arr, int [][] b, int [][] c, int st, Integer [][] dp, int k, int [] max){
        if(dp[st][k] != null) return dp[st][k];
        if(st == arr.length-1){
            if(k == 0) return dp[st][0] = b[st][arr[st]-1];
            return max[st];
        }
        if(k == 0){
            return dp[st][0] = b[st][arr[st]-1] + maxPoints(arr, b, c, st+1, dp, 0, max);
        }
        int without = maxPoints(arr, b, c, st+1, dp, k, max) + b[st][arr[st]-1];
        int with = 0;
        if(max[st] > b[st][arr[st]-1]) with = maxPoints(arr, b, c, st+1, dp, k-1, max) + max[st];
        return dp[st][k] = Math.max(without, with);
    }
