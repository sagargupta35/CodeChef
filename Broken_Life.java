...
class CodeChef{
    ...
    public static void main (String[] args) throws java.lang.Exception {
        ...
        while(t-- > 0) {
            ...
            int st = 0;
            Function<Character, Character> fun = c -> {
                for(char k : List.of('a', 'b', 'c', 'd', 'e')) if(c != k) return k;
                return c;
            };
            for(int i = 0; i<n && st < m; i++){
                if(sbr.charAt(i) == a.charAt(st)) st++;
                else if(sbr.charAt(i) == '?'){
                    sbr.setCharAt(i, fun.apply(a.charAt(st)));
                }
            }
            if(st >= m) out.println(-1);
            else out.println(sbr);
        }
        out.close();
    }
}
