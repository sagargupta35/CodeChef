...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            while (n-- > 0){
                int [] inv = findInversions(in.nextLine());
                sum += inv[0];
                map.merge(inv[1], 1, Integer::sum);
            }
            long k = 0;
            for(var entry: map.entrySet()){
                int z = entry.getKey();
                int f = entry.getValue();
                sum += z * (k*f + (m-z)*(sumOfNumbers(0, f-1)));
                k += (long) (m - z) *f;
            }
    ...
    static int [] findInversions(String s){
        int n = s.length();
        int [] suf = new int [n];
        if(s.charAt(n-1) == '0') suf[n-1] = 1;
        for(int  i= n-2; i>=0; i--){
            suf[i] = suf[i+1] + (s.charAt(i) == '0' ? 1 : 0);
        }
        int inversions = 0;
        for(int i = 0; i<n-1; i++){
            if(s.charAt(i) == '1') inversions += suf[i];
        }
        return new int [] {inversions, suf[0]};
    }
