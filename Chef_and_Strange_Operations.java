...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            long prev = 1, prev2 = 1;
            long sum = 0;
            for(int i = x-1, fac = 0; i>=0; i--, prev = (prev%mod * (m+fac)%mod)%mod, prev2 = (prev2 * (fac+1)%mod)%mod, fac++) {
                sum = (sum +  (arr[i]%mod * prev%mod * (modularMultiplicativeInverse(prev2, mod)))%mod)%mod;
            }
