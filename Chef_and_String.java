...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        String s = in.nextLine();
            ...
            int ci = c.size()-1, hi = h.size()-1, ei = e.size()-1, fi = f.size()-1;
            int res = 0;
            while (ci >= 0 && hi >= 0 && ei >= 0 && fi >= 0){
                int j1 = f.get(fi);
                while (ei >= 0 && e.get(ei) > j1) ei--;
                if(ei < 0) break;
                int j2 = e.get(ei);
                while (hi >= 0 && h.get(hi) > j2) hi--;
                if(hi < 0) break;
                int j3 = h.get(hi);
                while (ci >= 0 && c.get(ci) > j3) ci--;
                if(ci < 0) break;
                res++;
                fi--; ei--; hi--; ci--;
            }
