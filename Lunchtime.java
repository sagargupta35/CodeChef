...
class CodeChef{
    ...
    public static void main (String[] args) throws java.lang.Exception {
        ...
        while(t-- > 0) {
            int n = in.nextInt();
            int [] arr = ai(n);
            Map<Integer, Integer> [] pre = new HashMap [n];
            Map<Integer, Integer> [] suf = new HashMap [n];
            Arrays.setAll(pre, i -> new HashMap<>());
            Arrays.setAll(suf, i -> new HashMap<>());
            if(arr.length == 1){
                System.out.println(0);
                continue;
            }
            StringBuilder sbr = new StringBuilder();
            sbr.append(count(arr, 1, arr[0], suf, true)).append(" ");
            for(int i = 1; i<n-1; i++){
                int preCount = count(arr, i-1, arr[i], pre, false);
                int sufCount = count(arr, i+1, arr[i], suf, true);
                pre[i].put(arr[i], preCount+1);
                sbr.append((preCount+sufCount)).append(" ");
            }
            sbr.append(count(arr, n-2, arr[n-1], pre, false));
            System.out.println(sbr);
        }
        out.close();
    }
    
    static int count(int [] arr, int st, int target, Map<Integer, Integer> [] dp, boolean inc){
        if(st == 0){
            if(arr[0] == target) return 1;
            return 0;
        } else if(st == arr.length-1){
            if(arr[arr.length-1] == target) return 1;
            return 0;
        }
        if(arr[st] > target) return 0;
        if(dp[st].get(target) != null) return dp[st].get(target);
        int sum = arr[st] == target ? 1 : 0;
        sum += count(arr, inc ? st+1 : st-1, target, dp, inc);
        dp[st].put(target, sum);
        return sum;
    }
}
