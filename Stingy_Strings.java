...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            NavigableMap<Character, Integer> map = new TreeMap<>(Comparator.reverseOrder());
            for(int i = 0; i<n; i++){
            ...
            for(var entry : map.entrySet()){
                int val = entry.getKey()-'a'+1;
                int j = entry.getValue();
                while (j-- > 0){
                    temp -= 2;
                    sum += val;
                    max = Math.max(max, temp*k + sum);
                }
            }
            long gcd = gcd(max, k);
            System.out.println((max/gcd) + " " + (k/gcd));
        }
        out.close();
    }
