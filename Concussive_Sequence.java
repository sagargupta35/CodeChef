...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            left = 0; right = n/2; k = 0; flag = false;
            while (k < n){
                if(flag) res[k++] = arr[left++];
                else res[k++] = arr[right++];
                flag = !flag;
            }
            if(isConcussive(res)){
    ...
    static boolean isConcussive(int [] res){
        int n = res.length;
        int i = 1, flag = isConcussive(res, 1);
        while (flag != 0 && i < n-1){
            int j = isConcussive(res, i);
            if(j != flag){
                flag = 0;
                break;
            }
    ...
    static int isConcussive(int [] res, int i){
        /* return 0 if invalid,
         1 if ith element is mountain,
         2 if ith element is valley*/
        if(res[i] > res[i-1] && res[i] > res[i+1]) return 1;
        if(res[i] < res[i-1] && res[i] < res[i+1]) return 2;
        return 0;
    }
