...
class CodeChef3 {
    ...
    static class Illness implements Comparable<Illness>{
        int data;
        int index;
        public Illness(int data, int index){
            this.data = data;
            this.index = index;
        }
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            int n = in.nextInt();
            int [] arr = ai(n);
            Illness [] res = new Illness[n];
            for(int i = 0; i<n; i++){
                res[i] = new Illness(arr[i], i);
            }
            Arrays.sort(res);
            int st = 1;
            for(int i = 0; i<n; i++){
