...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        int [] arr = ai(n);
        int [] pre = new int [n];
        int prev = arr[0] < arr[1] ? 1 : -1, count = 0;
        for(int i = 1; i<n-1; i++) {
            if(arr[i] < arr[i+1]){
                if(prev == 1) pre[i] = count;
                else{
                    prev = 1;
                    pre[i] = ++count;
                }
        ...
        while (q-- > 0){
            ...
            int j = pre[r] - pre[l];
            if(pre[r] != pre[r-1]) j--;
            if((j&1) == 1) out.println("YES");
            else out.println("NO");
        }
        out.close();
    }

    public static int min_operations(int [] nums, int prev, int st){
        // Code here
        if(st == 0){
            if(nums[0] < prev) return 0;
            return 1;
        }
        if(nums[st] >= prev) return min_operations(nums, prev-1, st-1)+1;
        return Math.min(min_operations(nums, nums[st], st-1), min_operations(nums, prev-1, st-1)+1);
    }
