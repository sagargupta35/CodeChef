...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            System.out.printf("%.6f%n", probability(bools, p, a, b, 0, n));
        }
        out.close();
    }
    
    static double probability(boolean [] bools, int [] p, int [] a, int [] b, int st, int n){
        if(st == n-1){
            if(!bools[a[st]] && !bools[b[st]]) return 1;
            if(!bools[a[st]]) return ((double) p[st] )/100;
            if(!bools[b[st]]) return (100d - p[st])/100;
            return 0;
        }
        if(bools[a[st]] && bools[b[st]]) return 0;
        double d1 = 0, d2 = 0;
        if(!bools[a[st]]){
            bools[a[st]] = true;
            d1 = probability(bools, p, a, b, st+1, n)*(((double) p[st])/100);
            bools[a[st]] = false;
        }
        if(!bools[b[st]]){
            bools[b[st]] = true;
            d2 = probability(bools, p, a, b, st+1, n)*((100d-p[st])/100);
            bools[b[st]] = false;
        }
        return d1+d2;
    }
