...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<n; i++){
                ...
                int time = ((Integer.parseInt(s.substring(2)))/3)-1;
                int show = s.charAt(0)-'A';
                arr[time][show]++;
            }
            boolean [] cost = new boolean[4];
            boolean [] show = new boolean[4];
            long j = getMaxProfit(cost, show, arr, 0);
            sum += j;
    ...
    static long getMaxProfit(boolean [] cost, boolean [] show, int [][] arr, int time){
        if(time == 3){
            long max = -100;
            for(int i = 0; i<4; i++){
                if(arr[time][i] > 0 && !show[i]){
                    for(int j = 3; j>=0; j--){
                        if(!cost[j]){
                            max = arr[time][i] * 25L * (j+1);
                            break;
                        }
                    }
                }
        ...
        for(int i = 0; i<4; i++){
            if(!show[i]) {
                ...
                for (int j = 0; j < 4; j++) {
                    if(!cost[j]) {
                        cost[j] = true;
                        long temp = arr[time][i] > 0 ? 25L * arr[time][i] * (j + 1) : -100;
                        max = Math.max(max, getMaxProfit(cost, show, arr, time+1)+temp);
                        cost[j] = false;
                    }
                }
                show[i] = false;
