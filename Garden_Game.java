...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i<n; i++){
                if(!visited[i]){
                    list.add(connectedLength(g, i, visited));
                }
            }
            ...
            System.out.println(lcmOfArray(res, 100000));
        }
        out.close();
    }
    
    static int connectedLength(Graph g, int vertex, boolean [] visited){
        if(visited[vertex]) return 0;
        visited[vertex] = true;
        int res = 1;
        for(int j : g.edges[vertex]){
            res += connectedLength(g, j, visited);
        }
        return res;
    }
