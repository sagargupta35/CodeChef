...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<n; i++){
                if(arr[i] == 1) f++;
                else if((arr[i]&1) == 1) prod = (prod*(arr[i])%mod)%mod;
                else map.merge(arr[i], 1, Integer::sum);
            }
            for(var entry : map.entrySet()) {
                if(entry.getValue() <= f){
                    prod = (prod * pow(entry.getKey()+1, entry.getValue(), mod))%mod;
                    f -= entry.getValue();
                } else if(f > 0){
                    prod = (prod * pow(entry.getKey()+1, f, mod))%mod;
                    prod = (prod * pow(entry.getKey(), entry.getValue()-f, mod))%mod;
                    f = 0;
                } else{
                    prod =( prod * pow(entry.getKey(), entry.getValue(), mod))%mod;
                }
            }
            out.println(prod);
        }
