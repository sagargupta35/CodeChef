...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        int n = in.nextInt();
            int m = in.nextInt();
            int[] arr = ai(m);
            int[] toBeVisited = new int[n + 1];
            for (int i = 0; i < m; i++) toBeVisited[arr[i]-1] = 1;
            int time = 1, min = Integer.MAX_VALUE, min2 = -1;
            int[] cur = new int[n];
            for (int i = 1; i<n; i++, time++) {
                int j = 0, temp = 0, res = 0;
                while (temp < m && cur[j] != time) {
                    cur[j] = time;
                    if (toBeVisited[j] == 1) temp++;
                    j = (j + i) % n;
                    res++;
                }
                if (temp == m){
