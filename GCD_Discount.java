...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            int max = prev; // no discount applied
            prev = 0;
            for(int i = 0; i<n-1; i++){ // applying discount for ith item
                max = (int) Math.max(max, gcd(gcd(prev, b[i]), suf[i+1]));
                prev = pre[i];
            }
            max = (int) Math.max(gcd(pre[n-2], b[n-1]), max); // discount for last item
            out.println(max);
        }
        out.close();
    }
