...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            for(int i = 0; i<n; i++){
                int finalI = i;
                map.merge(arr[i], new ArrayList<>(List.of(i)), (old, val) -> {
                    old.add(finalI);
                    return old;
                });
            }
            long min = Long.MAX_VALUE, res = Integer.MAX_VALUE;
            for(var entry: map.entrySet()){
                List<Integer> list = entry.getValue();
                int sz = list.size();
                long sum = list.get(0);
                for(int j = 1; j<sz; j++){
                    sum = Math.max(sum, (list.get(j)-list.get(j-1))/2);
                }
                sum = Math.max(sum, n-list.get(sz-1)-1);
                if(sum < min){
                    min = sum;
                    res = entry.getKey();
                } else if(entry.getKey() < res && sum == min){
                    min = sum;
                    res = entry.getKey();
                }
            }
