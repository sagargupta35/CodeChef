...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            int top2 = 0;
            while (k < n && top > 1){
                min = (a[k] + b[(stack[0]+1)%n])%n;
                for(int i = 0; i<top; i++){
                    int j = (a[k] + b[(stack[i]+1)%n])%n;
                    if(j == min){
                        stack2[top2] = (stack[i]+1)%n;
                        top2++;
                    } else if(j < min){
                        top2 = 0;
                        stack2[top2] = (stack[i]+1)%n;
            ...
            //k-1th element should match to stack[0]
            k--;
            int st = (stack[0]-k+n)%n;
            for(int i = 0; i<n; i++){
                System.out.print((a[i] + b[(st+i)%n])%n + " ");
            }
            System.out.println();
        }
        out.close();
