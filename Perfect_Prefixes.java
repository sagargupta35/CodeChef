...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<n-1; i++){
                while (!pq.isEmpty() && pq.peek() <= (i+1)) pq.poll();
                if(arr[i] <= i+1){
                    if(pq.isEmpty()) pre[i] = -1;
                } else{
                    if(pq.isEmpty()){
                        if(arr[i+1] <= i+1) pre[i] = arr[i+1];
                    }
                    pq.add(arr[i]);
                }
            }
            ...
            int max = 0;
            int preOdd = 0; // l is even and r is idd
            int preEven = 0; // l is odd and r is even
            for(int i = 0; i<n; i++){
                int prev = 0, cur = 0, suf = 0;
                if(i > 2) prev = pr[i-2];
                if(i < n-1) suf = sf[i+1];
                if(pre[i] == -1) cur++;
                if(i > 0 && pre[i-1] == arr[i]) cur++;
                if((i&1) == 0){
                    preEven = Math.max(preEven, prev);
                    preEven += cur;
                    max = Math.max(max, preEven+suf);
                } else{
                    preOdd = Math.max(preOdd, prev);
                    preOdd += cur;
                    max = Math.max(max, preOdd+suf);
                }
            }
