...
class CodeChef{
    ...
    public static void main (String[] args) throws java.lang.Exception {
        ...
        while(t-- > 0) {
            int n = in.nextInt();
            int [] arr = ai(n);
            Map<Integer, List<Integer>> tel = new HashMap<>();
            Map<Integer, Integer> pop = new HashMap<>();
            for(int i = 0; i<n-1; i++){
                int v = in.nextInt()-1;
                int u = in.nextInt()-1;
                tel.merge(v, new ArrayList<>(List.of(u)), (old, val) -> {
                    old.add(u);
                    return old;
                });
                tel.merge(u, new ArrayList<>(List.of(v)), (old, val) -> {
                    old.add(v);
                    return old;
                });
            }
            ...
            for(int i = 0; i<n; i++){
                List<Integer> list = tel.get(i);
                int max = 0;
                if(list == null){
                    out.print((pop.get(arr[n-1])+1) + " ");
                    continue;
                }
                else {
                    rounds[i] = i+1;
