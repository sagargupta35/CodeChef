...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            long f = xor(arr, 1, dp, arr[0], 0, x);
            long s = xor(arr, 1, dp,  arr[0]+x, 1, x);
            System.out.println(Math.max(f, s));
        }
        out.close();
    }
    
    static long xor(long [] arr, int st, Long [][] dp, long prev, int did, long x){
        if(st == arr.length-1){
            return Math.max(prev^arr[st], prev ^ (arr[st]+x));
        }
        if(dp[st][did] != null) return dp[st][did];
        long f = xor(arr, st+1, dp, arr[st], 0,  x) + (prev^arr[st]);
        long s = xor(arr, st+1, dp, arr[st]+x, 1, x) +  (prev ^ (arr[st]+x));
        dp[st][did] = Math.max(f, s);
        return dp[st][did];
    }
