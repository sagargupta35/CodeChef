        ...
        while (t-- > 0){
            ...
            if(n == 1 || m == 1){
                if(n == 1){
                    ...
                    if(m == 1){
                        System.out.println(s.charAt(0) == '*' ? 0 : 1);
                        continue;
                    }
                    if((m&1) == 1){
                        boolean flag = true;
                        for(int i = 0; i<m; i++, flag = !flag){
                            if(flag && s.charAt(i) == '.') sum++;
                            else if(!flag && s.charAt(i) == '*') sum++;
                        }
                        System.out.println(sum);
                        continue;
                    }
                    System.out.println(getMinComps(s));
                } else {
                    StringBuilder s = new StringBuilder(n);
                    for(int i = 0; i<n; i++) s.append(in.nextLine());
                    if((n&1) == 1){
                        boolean flag = true;
                        for(int i = 0; i<m; i++, flag = !flag){
                            if(flag && s.charAt(i) == '.') sum++;
                    ...
                    System.out.println(getMinComps(s.toString()));
                }
                continue;
            }
            //true -> *
            ...
            for(int i = 0; i<n; i++, flag = !flag){
                boolean flag2 = flag;
                String s = in.nextLine();
                for(int j = 0; j<m; j++, flag2 = !flag2){
                    if(flag2 && s.charAt(j) == '.') sum++;
                    else if(!flag2 && s.charAt(j) == '*') sum++;
                }
            }
            if((m&1) == 0 || (n&1) == 0) System.out.println(Math.min(sum, (long) m *n-sum));
            else System.out.println(sum);
        }
        out.close();
    }
    
    static int getMinComps(String s){
        int sum = 0, n = s.length();
        boolean flag = true;
        for(int i = 0; i<s.length()-2; i++, flag = !flag){
            if(s.charAt(i) == '*' && !flag) sum++;
            else if(s.charAt(i) == '.' && flag) sum++;
        }
        if(s.charAt(n-1) == '.' && s.charAt(n-2) == '.') sum++;
        int sum2 = 0;
        flag = false;
        for(int i = 2; i<s.length(); i++, flag = !flag){
            if(s.charAt(i) == '.' && flag) sum2++;
            else if(s.charAt(i) == '*' && !flag) sum2++;
        }
        if(s.charAt(0) == '.' && s.charAt(1) == '.') sum2++;
