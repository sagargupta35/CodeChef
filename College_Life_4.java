...
class CodeChef3 {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0){
            ...
            for(int i = 0; i<=Math.min(n, Math.min(e, h)); i++){
                int e1 = e-i, h1 = h-i;
                if(a < b){
                    int temp1 = Math.min(n-i, e1/2);
                    int temp2 = Math.min(n-i-temp1, h1/3);
                    if(temp1+temp2+i == n){
                        ans = Math.min(ans, i*(long)c + temp1 * (long) a + temp2 * (long)b);
                    }
                } else{
                    int temp1 = Math.min(n-i, h1/3);
                    int temp2 = Math.min(n-i-temp1, e1/2);
                    if(temp1+temp2+i == n){
                        ans = Math.min(ans, i*(long)c + temp1 * (long) b + temp2 * (long)a);
                    }
                }
            }
